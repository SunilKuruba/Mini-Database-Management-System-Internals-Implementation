...
extern RC shutdownRecordManager ()
{
	...
	{
		// Ensure any buffer pool associated with the record manager is shut down
		if (recordManager->bufferPool.mgmtData != NULL)
...
extern RC openTable (RM_TableData *rel, char *name)
{
    ...
    // Ensure global record manager and its buffer pool are initialized
    if (recordManager == NULL)
    {
        recordManager = (RecordManager*) malloc(sizeof(RecordManager));
        memset(recordManager, 0, sizeof(RecordManager));
    ...
    for(k = 0; k < schema->numAttr; k++)
    {
        // Setting attribute name and ensure null-termination
        strncpy(schema->attrNames[k], pageHandle, ATTRIBUTE_SIZE);
        schema->attrNames[k][ATTRIBUTE_SIZE - 1] = '\0';
        pageHandle = pageHandle + ATTRIBUTE_SIZE;
...
extern RC insertRecord (RM_TableData *rel, Record *record)
{
	...
	*slotPointer = '+';
	...
	// Persist updated table info (tuple count and free page) to header page 0
	pinPage(&recordManager->bufferPool, &recordManager->pageHandle, 0);
	char *hdr = recordManager->pageHandle.data;
	*(int*)hdr = recordManager->tuplesCount; // total tuples
	hdr = hdr + sizeof(int);
	*(int*)hdr = recordManager->freePage; // first free page pointer
	markDirty(&recordManager->bufferPool, &recordManager->pageHandle);
	unpinPage(&recordManager->bufferPool, &recordManager->pageHandle);
...
extern RC deleteRecord (RM_TableData *rel, RID id)
{
	...
	*data = '-';
	...
	// Unpin the page after the record is marked deleted
	unpinPage(&recordManager->bufferPool, &recordManager->pageHandle);

	// Decrement tuple count and persist to header
	recordManager->tuplesCount--;
	pinPage(&recordManager->bufferPool, &recordManager->pageHandle, 0);
	char *hdr2 = recordManager->pageHandle.data;
	*(int*)hdr2 = recordManager->tuplesCount;
	hdr2 = hdr2 + sizeof(int);
	*(int*)hdr2 = recordManager->freePage;
	markDirty(&recordManager->bufferPool, &recordManager->pageHandle);
	unpinPage(&recordManager->bufferPool, &recordManager->pageHandle);
...
extern RC next (RM_ScanHandle *scan, Record *record)
{
    ...
    // If no condition, we scan all tuples
	...
	{  
		...
		*dataPointer = '-';
        ...
        // Test the record for the specified condition (if any)
        bool matches = TRUE;
        if (scanManager->condition != NULL)
		{
            RC evalRC = evalExpr(record, schema, scanManager->condition, &result);
            if (evalRC != RC_OK)
            {
                if (result != NULL)
                {
