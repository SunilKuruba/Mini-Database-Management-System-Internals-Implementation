...
#define ENSURE_SIZE(var,newsize)				\
		do {								\
			if ((var)->bufsize < (size_t)(newsize))					\
			{								\
				size_t newbufsize = (var)->bufsize;				\
				while((newbufsize *= 2) < (size_t)(newsize));			\
...
serializeTableInfo(RM_TableData *rel)
{
	...
	char *schemaStr = serializeSchema(rel->schema);
...
serializeTableContent(RM_TableData *rel)
{
	...
	{
		char *recStr = serializeRecord(r, rel->schema);
		APPEND_STRING(result, recStr);
